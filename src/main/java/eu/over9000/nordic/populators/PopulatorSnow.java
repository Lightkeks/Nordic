...
public class PopulatorSnow extends BlockPopulator {
        ...
        public void populate(final World world, final Random random, final Chunk source) {
                for (int x = 0; x < 16; x++) {
                        for (int z = 0; z < 16; z++) {
                                final int absX = x + source.getX() * 16;
                                final int absZ = z + source.getZ() * 16;
                                ...
                                if (snowSpot.getY() > 60 && snowSpot.getType() == Material.AIR) {
                                        ...
                                        if (base != Material.WATER && base != Material.LAVA) {
                                                ...
                                                if (data instanceof Snow snowData) {
                                                        // ensure snow layers remain within the valid range (1-8)
                                                        final int layers = 1 + random.nextInt(4);
                                                        snowData.setLayers(layers);
                                                        snowSpot.setBlockData(snowData, false);
                                                }
                                        }
                                }
                        }
